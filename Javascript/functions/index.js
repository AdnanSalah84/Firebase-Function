...
exports.onFinalize = functions.storage.object().onFinalize((object) => {
    ...
    console.info("fileBucket - ", fileBucket);
...
exports.updateText = functions.database.ref('texts/{textId}').onUpdate((snapshot, context) => {
    ...
    const uppercase = snapshot.after.val().value.toUpperCase();
    return snapshot.after.ref.child('uppercase').set(uppercase);
});
...
exports.changeText = functions.database.ref('texts/mytext').onWrite((snapshot, context) => {
    console.log("mytext was changed");
})
